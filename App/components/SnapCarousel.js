...
const SnapCarousel = (props) => {
    ...
    useEffect(() => {
        if (props.autoScroll) {
            interval = setInterval(
                () => {
                    if (
                        Math.round(scrollX.__getValue() % props.itemSize < 10)
                    ) {
    ...
    const onScroll = (e) => {
        if (
            Math.abs(scrollX.__getValue() - e.nativeEvent.contentOffset.x) > 0.5
        ) {
    ...
    const onScrollEnd = (e, props) => {
        var currentNextItem = props.horizontalScroll
            ? Math.round(e.nativeEvent.contentOffset.x / props.itemSize)
                    ...
                    style={[
                        {
                            ...
                            marginTop: props.indicatorSpacing
                                ? props.indicatorSpacing
                                : 10,
                        },
                        styles.dotContainer,
                    ]}>
